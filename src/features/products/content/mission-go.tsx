...
import { ProductImageGallery, ProductInfoForm } from '@/src/features/products/components';

export default function MissionGoStory() {
  ...
  return (
    <main className="bg-canvas text-primary w-full min-h-screen font-serif">
      ...
      <div className="w-full max-w-[3840px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-4 md:gap-6 xl:gap-8 2xl:gap-10 px-6 lg:px-12 relative items-start pt-6 md:pt-10 lg:pt-8 pb-10 lg:pb-16">
        ...
        <ProductImageGallery images={PRODUCT_IMAGES} modelName="MISSION GO" className="col-span-1 lg:col-span-7 xl:col-span-8" />

        {/* Right Column: Sticky Product Info Form (Takes 5 cols on lg, 4 cols on xl) */}
          <ProductInfoForm
          productId="mission-go"
          name="MISSION GO"
          category="MISSION"
          categoryHref="/products/mission"
          price="THB 25,000"
          description="Engineered for precision and speed. The MISSION GO is a high-performance robot optimized for maximum agility. Built specifically to dominate in competition categories, it delivers flawless execution on the most complex trajectories."
          inStock={true}
        />
      ...
      {/* -------------------------------------------------------------
          ...
          ------------------------------------------------------------- */}
      <section id="product-details-section" className="section-py px-6 lg:px-12 bg-canvas text-primary max-w-[1720px] mx-auto border-t border-hairline">
        <motion.h2 
          initial="hidden"
          whileInView="visible"
          ...
          <AnimatePresence>
            {!isExpanded && (
              <motion.div 
                ...
                className="absolute bottom-0 w-full h-[200px] bg-canvas/90 flex items-end justify-center z-20 pointer-events-none pb-0"
              >
                <button
        ...
        {/* Expanded Blocks Physics & Caching Maintained */}
        <motion.div 
          initial={false}
          animate={{ 
        ...
        >
          ...
          >
            <motion.div variants={childVariants} className="relative w-full aspect-[4/3] md:aspect-auto h-full min-h-[300px] max-md:order-2 max-md:min-h-0 max-md:my-0 max-md:mx-auto">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="https://assets.zyrosite.com/cdn-cgi/image/format=auto,w=768,h=817,fit=crop/Y4LDGNyengfoZkEX/btn_0-ALpPMJZ5egtlp3QR.png" alt="Last-generation drivers" className="w-full h-full object-cover rounded-[0px] max-md:my-0 my-4 md:my-6 lg:my-0 xl:-my-16" referrerPolicy="no-referrer" />
            </motion.div>
      ...
      {/* -------------------------------------------------------------
          ...
          ------------------------------------------------------------- */}
      <ShopCollections title="CUSTOMERS ALSO BOUGHT" defaultFilter="MISSION" />
    </main>
  );
}
