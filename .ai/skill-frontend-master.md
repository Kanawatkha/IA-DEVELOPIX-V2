...
- **Global Promotion of Shared Assets:** If a TypeScript type, utility helper, or custom hook is reused across multiple features or layout modules, it must not be duplicated inside local scopes. Promote it to the global shared folders (`src/hooks/`, `src/types/`, or `src/utils/`) to maintain a single source of truth.